defmodule Estuary.Anchor.Borsh do
  ...
  defp decode_type(<<value::little-unsigned-size(8), rest::binary>>, "u8", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-unsigned-size(16), rest::binary>>, "u16", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-unsigned-size(32), rest::binary>>, "u32", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-unsigned-size(64), rest::binary>>, "u64", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-signed-size(8), rest::binary>>, "i8", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-signed-size(16), rest::binary>>, "i16", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-signed-size(32), rest::binary>>, "i32", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-signed-size(64), rest::binary>>, "i64", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::little-signed-size(128), rest::binary>>, "i128", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::float-little-size(32), rest::binary>>, "f32", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<value::float-little-size(64), rest::binary>>, "f64", _types) do
    {:ok, value, rest}
  end

  defp decode_type(<<0, rest::binary>>, "bool", _types) do
    {:ok, false, rest}
  end

  defp decode_type(<<1, rest::binary>>, "bool", _types) do
    {:ok, true, rest}
  end
  ...
  defp decode_type(<<pubkey::binary-size(32), rest::binary>>, type, _types)
       when type in ["publicKey", "pubkey"] do
    {:ok, Base58.encode(pubkey), rest}
  end
  ...
  defp decode_type(<<0, rest::binary>>, %{"option" => _elem_type}, _types) do
    {:ok, nil, rest}
  end

  defp decode_type(<<_tag, rest::binary>>, %{"option" => elem_type}, types) do
    decode_type(rest, elem_type, types)
  end

  defp decode_type(bin, %{"array" => [elem_type, count]}, types) do
    decode_n(bin, elem_type, types, count, [])
  end
